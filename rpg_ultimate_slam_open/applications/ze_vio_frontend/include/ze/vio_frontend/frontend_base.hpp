...
#include <ze/vio_common/nframe.hpp>           // NFrame, NFramePtr
#include <ze/vio_common/frame.hpp>            // Frame, FramePtr (senin dizinde neyse o)
...
public:
  ...
  static cv::Mat& getPrevFrame();
  static cv::Mat& getCurrentFrame() ;

  static cv::Mat& getPrevImg();
  static cv::Mat& getCurrentImg() ;

  // .hpp dosyanın içine (FrontendBase class tanımına) ekle:
// mutable çünkü const olmayan bir fonksiyonda çağırmazsan hata verir
mutable std::deque<real_t> depth_history_;

  real_t estimateDepthFromOpticalFlow(const cv::Mat& prev_frame,
    const cv::Mat& curr_frame,
    double fx,
    double baseline_meters);
  
    real_t computeMedianDepthFromLandmarks();
...
private:
  ...
  void applyEdgeDetection(cv::Mat& out, const std::string& method);
