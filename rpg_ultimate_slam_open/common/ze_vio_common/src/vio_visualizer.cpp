  ...
  , stereo_pairs_(stereo_pairs)
{
  ...
  {
    stereo_pairs_ = StereoIndexPairs{{0, 1}};
  }
  else
  {
    stereo_pairs_ = stereo_pairs;
  ...
  pub_T_M_B_.reset(new ros::Publisher(nh_->advertise<geometry_msgs::PoseStamped>("T_M_B", 1)));
  pub_T_G_M_.reset(new ros::Publisher(nh_->advertise<geometry_msgs::PoseStamped>("T_G_M", 1)));
  pub_T_G_B_.reset(new ros::Publisher(nh_->advertise<geometry_msgs::PoseStamped>("T_G_B", 1)));
  pub_localization_state_.reset(new ros::Publisher(nh_->advertise<std_msgs::UInt32>("localization_state", 1)));
  pub_biases_.reset(new ros::Publisher(nh_->advertise<geometry_msgs::TwistStamped>("biases", 1)));
  pub_odom_.reset(new ros::Publisher(nh_->advertise<nav_msgs::Odometry>("odometry", 1)));
  pub_event_imgs_.reset(new image_transport::Publisher(it_->advertise("event_img", 1)));
}
...
{
  const Frame& frame_a = nframe.at(0);
  const Frame& frame_b = nframe.at(1);
  ...
  cv::Mat visualized;
  img_rgb.convertTo(visualized, CV_8U);  // float → 8-bit
  cv::normalize(visualized, visualized, 0, 255, cv::NORM_MINMAX);
  cv::imshow("Event Image VIOVIZ", visualized);
  cv::waitKey(10);
  static int frame_id = 0;
  if(frame_id % 10 ==0)
  {
    std::string filename = "/home/sebnem/event_frames/VIOVIZ/vio_" + std::to_string(frame_id) + ".png";
    ...
    const uint32_t level)
{
  ...
  for(uint32_t frame_idx : frame_indices)
  {
    ...
    cv::Mat img_pre = ImageCv8uC1(frame.pyr_->at(level)).cvMat();
    cv::Mat img;
    cv::resize(img_pre,img,cv::Size(img_size.width(),img_size.height()));
    cv::cvtColor(img, img_rgb, cv::COLOR_GRAY2RGB);
    if (FLAGS_vio_viz_show_image_statistics)
    {
  ...
  if (FLAGS_vio_viz_show_image)
  {
   ...
   //cv::Mat visualized;
   //img_rgb_all.convertTo(visualized, CV_8U);  // float → 8-bit
   //cv::normalize(visualized, visualized, 0, 255, cv::NORM_MINMAX);
   //cv::imshow("Event Image VIOVIZ", visualized);
   //cv::waitKey(10);
   //  //
