...
loadVioDetectorsFromGflags(const CameraRig& rig)
{
  ...
  else if (FLAGS_imp_detector_name == "ORB")
  {
    for (const Camera::Ptr& cam : rig)
    {
      DetectionStrategy strategy;
...
//          1.2f,
//          FLAGS_imp_detector_num_octaves,
//          FLAGS_imp_detector_border_margin,
//          8, 31,
//          cv::ORB::HARRIS_SCORE,
//          FLAGS_imp_detector_grid_size,
//          FLAGS_imp_detector_threshold
//      );
        auto orb = cv::ORB::create(
          FLAGS_imp_detector_max_features_per_frame,
          1.1f,
      ...
      strategy.emplace_back(
        std::make_shared<OrbDetector>(orb, cam->size()));
